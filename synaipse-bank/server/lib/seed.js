...
export function seedDatabase(db) {
  ...
  db.prepare(
    `INSERT INTO users (id,email,fullName,passwordHash,role,mfaEnabled,phoneNumber,creditScore,createdAt)
     VALUES (?,?,?,?,?,?,?,?,?)`,
  ).run(userId, "demo@synaipse.bank", "Demo Customer", bcrypt.hashSync("Demo123$", 10), "Customer", 0, "+1 415 555 0100", 781, now)
  ...
  insertAccount.run(checkingId, userId, "1000200030", "US12SYNAIPSE0001000200030", "Checking", "Active", "USD", 0, 0, "Everyday Checking")
  insertAccount.run(savingsId, userId, "4000500060", "US12SYNAIPSE0004000500060", "Savings", "Active", "USD", 0, 0, "Premier Savings")
  ...
  insertNote.run(uuid(), userId, "Welcome to SynAIpse Bank", "Your demo account is ready. Explore the dashboard and Symbolic AI!", 0, now)
  ...
  console.log("[seed] Demo data created \u2014 login with demo@synaipse.bank / Demo123$")
}
