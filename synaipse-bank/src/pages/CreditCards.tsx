    ...
    <>
      ...
      <Section>
        <div className="grid gap-6 md:grid-cols-3">
          {[
            { name: "SynAIpse Everyday", color: "from-slate-700 to-slate-900", perk: "1% cashback on all spend" },
            { name: "SynAIpse Rewards", color: "from-brand-600 to-brand-900", perk: "3% on travel & dining" },
            { name: "SynAIpse Metal", color: "from-accent-600 to-brand-800", perk: "Airport lounges + 5% boosted" },
          ].map((c) => (
            <div key={c.name} className="card">
              <div className={`mb-4 flex h-40 flex-col justify-between rounded-xl bg-gradient-to-br ${c.color} p-4 text-white`}>
                <span className="text-sm font-semibold">SynAIpse Bank</span>
