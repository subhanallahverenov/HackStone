...
export default function Login() {
  ...
  const [email, setEmail] = useState("demo@synaipse.bank")
  ...
  return (
    <div className="grid min-h-screen lg:grid-cols-2">
      <div className="relative hidden overflow-hidden bg-gradient-to-br from-brand-950 via-brand-800 to-brand-600 p-12 text-white lg:flex lg:flex-col lg:justify-between">
        ...
        <p className="relative text-xs text-brand-200">© {new Date().getFullYear()} SynAIpse Bank</p>
      </div>
      ...
      <div className="flex items-center justify-center bg-slate-50 px-4 py-12">
        <div className="w-full max-w-md">
          ...
          <p className="mb-6 mt-1 text-center text-sm text-slate-500">Sign in to your SynAIpse Bank account</p>
          ...
          <p className="mt-6 text-center text-sm text-slate-500">
            New to SynAIpse Bank?{" "}
            <Link to="/register" className="font-semibold text-brand-700">
              Open an account
            </Link>
          </p>
          <p className="mt-3 text-center text-xs text-slate-400">Demo: demo@synaipse.bank / Demo123$</p>
        </div>
      </div>
    </div>
  )
